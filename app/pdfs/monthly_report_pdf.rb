class MonthlyReportPdf
  ...
  def initialize(client, date, total)
    font_setup 
    @client = client
    @date = date
    @total = total
  ...
  def body_content
    ...
    bounding_box([0, y_position], :width => 540) do
      ...
      move_down 15
      text "Le informo que durante el mes de <b>#{@date.capitalize}</b> recolectamos la cantidad de <b>#{@total} Litros</b> de Aceite Vegetal Usado en nuestra visita a su establecimiento. Con lo cual demuestran su sólido compromiso con el medio ambiente, al darle una disposición controlada a este residuo.", inline_format: true, align: :justify, color: "959595"
      move_down 15
